...
namespace Blackjack.Views
{
    ...
    public partial class BlackjackPage : INotifyPropertyChanged
    {
        ...
        private int _gold, _index, _mainBet, _splitBet, _sidePot, _totalWins, _totalLosses, _totalDraws, _totalBetWinnings, _totalBetLosses;
        ...
        /// <summary>Current amount of gold on hand.</summary>
        public int Gold
        {
            get => _gold;
            set { _gold = value; NotifyPropertyChanged(nameof(Gold), nameof(GoldToString), nameof(GoldToStringWithText)); }
        }
        ...
        private void BtnDealHand_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MainBet > 0 && MainBet <= Gold)
                DealHand();
            else if (MainBet > Gold)
                DisplayNotification("You can't bet more gold than you have!", "Blackjack");
            else
                DisplayNotification("Please enter a valid bet.", "Blackjack");
        }
        ...
        private void BtnInsurance_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (sidePot <= MainBet / 2 && Gold >= MainBet + SplitBet + sidePot)
            {
                SidePot = sidePot;
                ToggleInsurance(false);
            }
            else
                DisplayNotification("Your insurance bet must be less than or equal to half your main bet.", "Blackjack");
        }

        private void BtnDoubleDown_Click(object sender, RoutedEventArgs e)
        {
            if (Gold >= (MainBet * 2) + SplitBet + SidePot)
            {
        ...
        {
            if (Gold >= MainBet + (SplitBet * 2) + SidePot)
            {
        ...
        internal void DisplayNotification(string message, string title) => Application.Current.Dispatcher.Invoke(() => new Notification(message, title, NotificationButton.OK, Window.GetWindow(this)).ShowDialog());
